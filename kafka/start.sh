...
KAFKA_INT_PORT=${KAFKA_INT_PORT:-"9092"}
ZOOKEEPER_CONNECTION_STRING=${ZOOKEEPER_CONNECTION_STRING:-"zookeeper:2181"}
KAFKA_ADVERTISED_PORT=${KAFKA_ADVERTISED_PORT:-"31092"}
KAFKA_ADVERTISED_HOST_NAME=${KAFKA_ADVERTISED_HOST_NAME:-"192.168.99.100"}
export KAFKA_LISTEN_PORT ZOOKEEPER_CONNECTION_STRING KAFKA_INT_PORT KAFKA_ADVERTISED_PORT KAFKA_ADVERTISED_HOST_NAME
...
if [ "$KAFKA_JMX" = "true" ]; then
  ...
  if [ -z "$KAFKA_JMX_OPTS" ]; then
    ...
    KAFKA_JMX_OPTS="$KAFKA_JMX_OPTS -Djava.rmi.server.hostname=$KAFKA_INT_HOST_NAME "
